<template>
<div class="swiper-wrap" @touchstart="touchStart" @touchmove.prevent="touchMove" @touchend="touchEnd">
	<div class="swiper-main" :class="[effect, {'swiper-page': !mode}]" ref="main" :style="{'transform': 'translate3d('+posX+'px,'+posY+'px,0)', 'transition-duration': transitionDuration}">
		<slot></slot>
	</div>
	<slot name="ex"></slot>
	<div v-if="indicator && mode == 0" class="swiper-indicator">
		<span v-for="(item, index) in iLength" :class="{'act': index == currentIndex}"></span>
	</div>
</div>
</template>
...
export default {
	...
	computed: {
		iDistance() {
			return this.effect == 'horizontal' ? this.$refs.main.offsetWidth : this.$refs.main.offsetHeight;
		},

		dir() {
			return this.effect == 'horizontal' ? 'X' : 'Y';
		}
	},
	props: {
		indicator: {
	...
	methods: {
		init() {
			const els = this.$refs.main.querySelectorAll('.swiper-item');
			let val = 0, dir, marginBefore, marginAfter;

			if (this.effect == 'horizontal') {
				dir = 'offsetWidth';
			...
			//防止swiper-item靠图片撑开大小，图片未加载完的情况
			if (!val) {
		...
		setAuto() {
			if (this.mode == 0 && this.iLength > 1) {
				...
				this.player = setInterval(this.toNext, this.auto);
			}
		},
...
<style scoped>
.swiper-wrap{display:flex; position:absolute; top:0; left:0; width:100%; height:100%; overflow:hidden;}
.swiper-main{display:-webkit-flex; position:relative; width:100%; height:100%;}
.vertical{-webkit-box-orient:vertical; box-orient:vertical; -webkit-box-direction:normal; box-direction:normal; -webkit-flex-direction:column; flex-direction:column;}
.swiper-main .swiper-item{-webkit-flex-shrink:0; flex-shrink:0;}
.swiper-page .swiper-item{width:100%; height:100%;}
</style>